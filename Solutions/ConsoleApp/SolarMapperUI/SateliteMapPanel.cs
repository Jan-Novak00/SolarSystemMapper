...
namespace SolarMapperUI
{
    internal class SateliteMapPanel : SolarSystemMapPanel
    {
        ...
        public SateliteMapPanel(List<ObjectEntry> objects, DateTime mapStartDate, NASAHorizonsDataFetcher.MapMode Mode, string CenterName,float scale_km = 1000) : base(objects, mapStartDate, scale_km)
        {
            this._mode = Mode;
        ...
        private void _drawArrowAtTheEdge(Graphics graphics, PointF location, Color color, string name)
        {
            ...
            if (direction.X != 0)
            {
                float tx1 = (0 - center.X) / direction.X; 
                float tx2 = (this.ClientRectangle.Width - center.X) / direction.X; 
            ...
            if (direction.Y != 0)
            {
                float ty1 = (0 - center.Y) / direction.Y; 
                float ty2 = (this.ClientRectangle.Height - center.Y) / direction.Y; 
